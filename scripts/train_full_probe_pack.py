#!/usr/bin/env python3
"""
Train complete SUMO probe pack including all layers and simplexes.
...
4. Generates probe pack ready for deployment

Architecture:
- Layers 0-5: Binary classifiers for hierarchical SUMO concepts
- Layer 2 simplexes: 3 binary probes per simplex (negative/neutral/positive poles)
- Total: ~5,665 regular probes + 39 simplex probes (3 per simplex × 13 simplexes)
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train complete SUMO probe pack with all layers and simplexes"
    )
    ...
    parser.add_argument('--output-dir', type=str,
                        default="results/full_probe_pack",
                        help='Output directory (default: results/full_probe_pack)')
...
def train_simplex_pole(
    ...
    # Train with adaptive trainer (activation probe only)
    result = trainer.train_activation_probe_adaptive(
    ...
    if result['classifier'] is not None:
        probe_path = run_dir / f"{dimension}_{pole_type}_pole.pt"
        torch.save(result['classifier'].state_dict(), probe_path)
...
def main():
    ...
    print("=" * 80)
    print("FULL PROBE PACK TRAINING")
    print("=" * 80)
    print(f"\nModel: {args.model}")
    ...
    print("\nNext steps:")
    print("1. Assemble probe pack: python scripts/assemble_probe_pack.py")
    print("2. Calibrate probes: python scripts/calibrate_probe_pack.py")
