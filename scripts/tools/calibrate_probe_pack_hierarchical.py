...
def evaluate_probe_hierarchical(
    ...
    with torch.inference_mode():
        ...
        single_term_logit = probe(single_term_activation)
...
def main():
    ...
    # Build concept lookup by layer - SCAN probe files directly for all available probes
    concepts_by_layer = defaultdict(list)
    total_probes_found = 0

    for layer_dir in sorted(probes_dir.glob("layer*")):
    ...
    print(f"Found {total_probes_found} probes across {len(concepts_by_layer)} layers:")
    for layer in sorted(concepts_by_layer.keys()):
        print(f"  Layer {layer}: {len(concepts_by_layer[layer])} probes")
    ...
    # Cache for loaded probes (load directly, not through manager)
    loaded_probes = {}

    for concept_name, layer, definitions, metadata in tqdm(all_concepts, desc="Testing probes"):
        probe_key = (concept_name, layer)

        # Load probe directly from file
        probe_file = probes_dir / f"layer{layer}" / f"{concept_name}_classifier.pt"
        if not probe_file.exists():
            continue

        if probe_key not in loaded_probes:
            try:
                probe_data = torch.load(probe_file, map_location=args.device)
                # Probe is a 3-layer MLP from BinaryClassifier:
                ...
                input_dim = probe_data['0.weight'].shape[1]
                hidden1 = probe_data['0.weight'].shape[0]
                hidden2 = probe_data['3.weight'].shape[0]
                output_dim = probe_data['6.weight'].shape[0]
                ...
                print(f"Failed to load probe {concept_name} L{layer}: {e}")
                continue

        probe = loaded_probes[probe_key]
        ...
        # Unload probe to free memory (keep cache small)
        if len(loaded_probes) > 100:
            # Clear oldest probes
            keys_to_remove = list(loaded_probes.keys())[:50]
