#!/usr/bin/env python3
"""
Profile DynamicProbeManager overhead to identify bottlenecks.

Measures timing for:
1. Model forward pass (baseline)
2. Probe detection only
3. Dynamic probe loading/unloading
...
def profile_model_baseline(model, tokenizer, prompt: str, n_tokens: int = 30, device: str = "cuda"):
    """Profile pure model generation without any probe overhead."""
    model.eval()
...
def profile_with_probe_detection(model, tokenizer, probe_manager, prompt: str, n_tokens: int = 30, device: str = "cuda"):
    """Profile with probe detection but minimal dynamic loading."""
    model.eval()
    ...
    return {
        ...
        'probe_detection_time': timings['probe_detection'],
        'per_token_ms': per_token,
        'tokens': n_tokens,
        'breakdown': {
            'generation_pct': timings['generation'] / timings['total'] * 100,
            'hidden_extraction_pct': timings['hidden_extraction'] / timings['total'] * 100,
            'probe_detection_pct': timings['probe_detection'] / timings['total'] * 100,
        }
    }


def profile_detect_and_expand_internal(probe_manager, hidden_state):
    """Profile internal detect_and_expand breakdown."""
    # Get detailed timing from probe_manager
    with torch.inference_mode():
        start = time.perf_counter()
        detected, timing_info = probe_manager.detect_and_expand(
            hidden_state,
    ...
    return {
        ...
        'probes_loaded': timing_info.get('probes_loaded', 0),
...
def main():
    print("=" * 80)
    print("DYNAMIC PROBE MANAGER - PERFORMANCE PROFILING")
    print("=" * 80)
    ...
    print("\n" + "-" * 80)
    print("3. INITIALIZING PROBE MANAGER")
    print("-" * 80)
    ...
    print(f"  Base probes loaded: {len(probe_manager.loaded_probes)}")

    # With probe detection
    print("\n" + "-" * 80)
    print("4. WITH PROBE DETECTION: Full pipeline")
    print("-" * 80)
    with_probes = profile_with_probe_detection(model, tokenizer, probe_manager, prompt, n_tokens, device)
    print(f"Total time: {with_probes['total_time']:.3f}s")
    print(f"Per-token: {with_probes['per_token_ms']:.2f}ms")
    print(f"\nBreakdown:")
    print(f"  Generation:        {with_probes['generation_time']:.3f}s ({with_probes['breakdown']['generation_pct']:.1f}%)")
    print(f"  Hidden extraction: {with_probes['hidden_extraction_time']:.3f}s ({with_probes['breakdown']['hidden_extraction_pct']:.1f}%)")
    print(f"  Probe detection:   {with_probes['probe_detection_time']:.3f}s ({with_probes['breakdown']['probe_detection_pct']:.1f}%)")

    probe_overhead = with_probes['per_token_ms'] - with_hidden['per_token_ms']
    print(f"\nProbe detection overhead: {probe_overhead:.2f}ms per token")

    # Profile single detect_and_expand call in detail
    print("\n" + "-" * 80)
    print("5. DETAILED PROBE DETECTION (single token)")
    print("-" * 80)
    ...
    internal = profile_detect_and_expand_internal(probe_manager, hidden_state)
    ...
    print(f"  Probes loaded: {internal['probes_loaded']}")
    ...
    print(f"+ Probe detection:          {with_probes['per_token_ms']:.2f}ms/token (+{probe_overhead:.2f}ms)")
    print(f"\nTotal HatCat overhead:      {with_probes['per_token_ms'] - baseline['per_token_ms']:.2f}ms/token")

    target_overhead = 10.0  # Target: <10ms per token
    current_overhead = with_probes['per_token_ms'] - baseline['per_token_ms']
