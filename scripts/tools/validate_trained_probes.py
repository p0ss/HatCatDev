#!/usr/bin/env python3
"""
Validate all trained probes to identify poorly-calibrated classifiers.

Usage:
    python scripts/validate_trained_probes.py --layer 1
    python scripts/validate_trained_probes.py --probe-pack gemma-3-4b-pt_sumo-wordnet-v1 --layer 1
"""
...
from src.training.probe_validation import validate_probe_set


def main():
    parser = argparse.ArgumentParser(
        description="Validate trained probes for calibration"
    )
    parser.add_argument('--probe-pack', type=str, default='gemma-3-4b-pt_sumo-wordnet-v1',
                       help='Probe pack ID')
    ...
    # Load probe pack metadata
    from src.registry import ProbePackRegistry
    registry = ProbePackRegistry()
    probe_pack = registry.get_pack(args.probe_pack)

    if not probe_pack:
        print(f"ERROR: Probe pack '{args.probe_pack}' not found")
        return 1

    # Get probes directory (all probes in one dir, we'll filter by layer)
    probe_dir = probe_pack.activation_probes_dir
