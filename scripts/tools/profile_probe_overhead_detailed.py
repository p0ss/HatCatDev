#!/usr/bin/env python3
"""
Detailed per-millisecond breakdown of DynamicProbeManager overhead.

Profiles:
1. Base probe inference (per probe)
2. Child loading (disk I/O + initialization)
3. Child probe inference
4. Cache management
...
def profile_single_token_detailed(model, tokenizer, probe_manager, prompt: str, device: str = "cuda"):
    ...
    return {
        ...
        'num_base_probes': len(probe_manager.loaded_probes) - timing_info.get('num_children_loaded', 0),
        'num_total_probes': len(probe_manager.loaded_probes),
        'concepts_detected': len(detected),
    }


def profile_base_probe_inference(probe_manager, hidden_state, num_runs=100):
    """Profile time to run inference on all base probes."""
    times = []
    ...
    per_probe = avg_time / len(probe_manager.loaded_probes)

    return {
        'avg_total_ms': avg_time,
        'per_probe_ms': per_probe,
        'num_probes': len(probe_manager.loaded_probes),
    }
...
def main():
    print("=" * 80)
    print("DETAILED PROBE OVERHEAD ANALYSIS")
    print("=" * 80)
    ...
    print("\n" + "-" * 80)
    print("INITIALIZING PROBE MANAGER (base_layers=[0,1])")
    print("-" * 80)
    ...
    detailed = profile_single_token_detailed(model, tokenizer, probe_manager, prompt, device)
    ...
    print(f"    └─ {detailed['num_base_probes']} base probes @ {detailed['initial_detection_ms']/detailed['num_base_probes']:.3f}ms/probe")
    ...
    if detailed['num_children_loaded'] > 0:
        print(f"    └─ {detailed['child_detection_ms']/detailed['num_children_loaded']:.3f}ms/probe")
    ...
    print(f"  Total probes run: {detailed['num_base_probes'] + detailed['num_children_loaded']}")

    # ========================================================================
    # 2. BASE PROBE INFERENCE MICROBENCHMARK
    # ========================================================================
    print("\n" + "=" * 80)
    print("2. BASE PROBE INFERENCE MICROBENCHMARK")
    print("=" * 80)

    print(f"\nRunning 100 iterations of {len(probe_manager.loaded_probes)} probes...")
    base_timing = profile_base_probe_inference(probe_manager, hidden_state, num_runs=100)

    print(f"\nResults:")
    print(f"  Average total time: {base_timing['avg_total_ms']:.2f}ms")
    print(f"  Per-probe time:     {base_timing['per_probe_ms']:.4f}ms")
    print(f"  Num probes:         {base_timing['num_probes']}")
    ...
    mock_scores = {(f"concept_{i}", i % 6): 0.5 + (i * 0.01) % 0.5
                   for i in range(len(probe_manager.loaded_probes))}
    ...
    print(f"\nProbe detection: {timings['probe_detection']*1000:.2f}ms ({timings['probe_detection']*1000/n_tokens:.2f}ms/token)")
    ...
    python_overhead = timings['probe_detection']*1000 - total_accounted
    ...
    per_token_overhead = timings['probe_detection'] * 1000 / n_tokens
    ...
    if components[0][0] == 'Initial detection':
        avg_per_probe = timings['initial_detection'] / n_tokens / len(probe_manager.loaded_probes)
        print(f"  • Initial detection: Batching could help (currently {avg_per_probe*1000:.3f}ms/probe)")
        print(f"    - Run all {len(probe_manager.loaded_probes)} probes in single batch instead of loop")
