#!/usr/bin/env python3
"""
Train text-based concept probes for fast token→SUMO mapping.
...
from src.monitoring.text_concept_probe import (
    TfidfConceptProbe,
...
def main():
    parser = argparse.ArgumentParser(
        description="Train fast text→concept probes"
    )
    parser.add_argument(
        '--mode',
    ...
    parser.add_argument(
        '--output-dir',
        type=Path,
        default=Path('results/text_probes'),
    ...
    print("="*80)
    print("TEXT CONCEPT PROBE TRAINING")
    print("="*80)
    print(f"\nMode: {args.mode}")
    ...
    if args.mode == 'from-classifier-data':
        ...
        metrics = train_text_probes_from_classifier_data(
            layers=args.layers,
        ...
        print(f"\n{'='*80}")
        print(f"TRAINING {args.probe_type.upper()} PROBE")
        print(f"{'='*80}")
        print(f"Total samples: {len(all_texts)}")
        print(f"Unique concepts: {len(set(all_concepts))}")

        if args.probe_type == 'tfidf':
            # Train TF-IDF probe
            ...
            # Save
            probe_path = args.output_dir / "tfidf_probe.joblib"
            ...
            for token in test_tokens:
                preds = probe.predict(token, top_k=3)
                print(f"\n'{token}':")
                for concept, prob in preds:
                    print(f"  {concept:30s} {prob:.3f}")

        else:
            print(f"Probe type '{args.probe_type}' not yet implemented")
            return
