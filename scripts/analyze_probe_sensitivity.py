#!/usr/bin/env python3
"""
Analyze Probe Sensitivity Issues

Diagnoses why probes are over-firing or under-firing by examining:
...
4. Common patterns among problematic probes
"""
...
def analyze_over_firing_probes(results: Dict, metadata: Dict):
    """Analyze why probes are over-firing."""
    print("\n" + "="*80)
    print("OVER-FIRING PROBES ANALYSIS")
    print("="*80)
    ...
    print(f"\nFound {len(over_firing)} over-firing probes\n")
    ...
    for key, data in sorted(over_firing, key=lambda x: x[1]['metrics']['fp_rate'], reverse=True):
        ...
        if meta:
            ...
            print(f"    Category Probe: {meta['is_category_probe']}")
        ...
        if metrics['avg_positive_score'] - metrics['avg_negative_score'] < 0.2:
            print(f"    ⚠️  Poor score separation - probe can't distinguish concept well")
        if meta and len(meta['category_children']) > 20:
            print(f"    ⚠️  Many children ({len(meta['category_children'])}) - very broad category")


def analyze_under_firing_probes(results: Dict, metadata: Dict):
    """Analyze why probes are under-firing."""
    print("\n" + "="*80)
    print("UNDER-FIRING PROBES ANALYSIS")
    print("="*80)
    ...
    print(f"\nFound {len(under_firing)} under-firing probes\n")
    ...
    for layer in sorted(by_layer.keys()):
        print(f"  Layer {layer}: {len(by_layer[layer])} probes")

    print("\nDistribution by Synset Count:")
    for count_range in ['0', '1-2', '3-9', '10+']:
        if count_range in by_synset_count:
            print(f"  {count_range} synsets: {len(by_synset_count[count_range])} probes")

    # Show detailed analysis for worst offenders
    print("\nTop 10 Worst Under-Firing Probes:")
    ...
    for key, data in sorted(under_firing, key=lambda x: x[1]['metrics']['tp_rate'])[:10]:
        ...
        if meta and meta['synset_count'] == 0:
            print(f"    ❌ NO training synsets - probe has no examples!")
        ...
        if metrics['avg_positive_score'] < 0.5:
            print(f"    ⚠️  Low positive scores - probe doesn't recognize its own concept")
...
def analyze_patterns(results: Dict, metadata: Dict):
    """Analyze common patterns across all probes."""
    print("\n" + "="*80)
...
def main():
    # Paths
    calibration_file = Path("results/probe_calibration/calibration_20251119_224541.json")
    ...
    print(f"\n✓ Loaded {results['total_probes']} probe calibration results")
    print(f"✓ Loaded metadata for {len(metadata)} concepts")

    # Analyze problematic probes
    analyze_over_firing_probes(results, metadata)
    analyze_under_firing_probes(results, metadata)
