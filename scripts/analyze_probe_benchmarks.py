#!/usr/bin/env python3
"""
Analyze and visualize probe benchmark results.
...
def create_confusion_matrix(
    ...
    """
    Create confusion matrix heatmap for concept probes.
    ...
    plt.title(f'Concept Probe Confusion Matrix ({context})')
    plt.tight_layout()
...
def plot_steering_effectiveness(
    ...
    """
    Plot steering effectiveness for concept probes.
    ...
    ax.set_title('Steering Effectiveness (Concept Probes)')
    ax.set_xticks(x)
...
def calculate_quality_metrics(df: pd.DataFrame) -> Dict:
    """
    Calculate quality metrics for concept probes.
...
def main():
    parser = argparse.ArgumentParser(description="Analyze probe benchmark results")
    parser.add_argument("--concept-csv", type=Path, help="Concept probe benchmark CSV")
    parser.add_argument("--simplex-csv", type=Path, help="Simplex probe benchmark CSV")
    parser.add_argument("--output-dir", type=Path, required=True, help="Output directory for plots/reports")
    args = parser.parse_args()

    print("=" * 80)
    print("PROBE BENCHMARK ANALYSIS")
    print("=" * 80)

    args.output_dir.mkdir(parents=True, exist_ok=True)

    # Analyze concept probes
    if args.concept_csv and args.concept_csv.exists():
        print(f"\nAnalyzing concept probes: {args.concept_csv}")
    ...
    # Analyze simplex probes
    if args.simplex_csv and args.simplex_csv.exists():
        print(f"\nAnalyzing simplex probes: {args.simplex_csv}")
