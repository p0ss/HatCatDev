#!/usr/bin/env python3
"""
Analyze Probe Calibration Results
...
def plot_category_distribution(summary: Dict, output_dir: Path):
    """Plot pie chart of probe categories."""
    ...
    ax.set_title(f"Probe Calibration Categories\n{summary['probe_pack']}", fontsize=14, fontweight='bold')
...
def plot_fp_tp_scatter(results: Dict, output_dir: Path):
    ...
    ax.set_title('Probe Performance: FP Rate vs TP Rate', fontsize=14, fontweight='bold')
...
def plot_f1_histogram(results: Dict, output_dir: Path):
    ...
    ax.set_ylabel('Number of Probes', fontsize=12)
...
def plot_score_distributions(results: Dict, output_dir: Path, sample_size: int = 20):
    ...
    # Sample random probes for visualization
...
def generate_detailed_report(summary: Dict, output_dir: Path):
    ...
    with open(report_file, 'w') as f:
        f.write("="*80 + "\n")
        f.write("PROBE PACK CALIBRATION REPORT\n")
        f.write("="*80 + "\n\n")

        f.write(f"Probe Pack: {summary['probe_pack']}\n")
        ...
        for cat, count in sorted(summary['categories'].items(), key=lambda x: -x[1]):
            pct = 100 * count / summary['total_probes']
            f.write(f"{cat:20s}: {count:5d} probes ({pct:5.1f}%)\n")

        # Well-calibrated probes
        f.write("\n" + "="*80 + "\n")
        f.write("WELL-CALIBRATED PROBES (Top 20)\n")
        f.write("="*80 + "\n\n")
        ...
        # Over-firing probes
        f.write("\n" + "="*80 + "\n")
        f.write("OVER-FIRING PROBES (Top 20 by FP Rate)\n")
        f.write("="*80 + "\n\n")
        ...
        # Under-firing probes
        f.write("\n" + "="*80 + "\n")
        f.write("UNDER-FIRING PROBES (Top 20 by Low TP Rate)\n")
        f.write("="*80 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze probe calibration results"
    )
    ...
    print("="*80)
    print("PROBE CALIBRATION ANALYSIS")
    print("="*80)
    ...
    print(f"✓ Loaded {summary['total_probes']} probe results")
