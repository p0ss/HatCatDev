"""
Adaptive Relationship-First Training to 95% Accuracy

Combines:
- Phase 2: Adaptive training loop (add data until 95% accuracy)
- Phase 3: Relationship-first generation (generate each edge once, reuse)

Strategy:
1. Start all concepts at baseline (e.g., 10 defs × 10 rels)
2. Use relationship-first: generate each edge once, reuse for both concepts
3. Test accuracy on fixed test set
4. Concepts above 95% accuracy graduate
5. Add incremental data to remaining concepts
6. Repeat until all concepts reach 95%

Key advantages:
- Efficient: Each relationship generated once, reused for multiple concepts
- Adaptive: Only train concepts that need more data
- Scalable: Works for 1 to 100k+ concepts
"""
...
class AdaptiveRelationshipFirstScaler:
    """Manages adaptive scaling with relationship-first generation."""
    ...
    def __init__(
        ...
        increment_defs: int = 5,
        increment_rels: int = 5,
        layer_idx: int = -1,
        device: str = "cuda",
        ...
        self.increment_defs = increment_defs
        self.increment_rels = increment_rels
    ...
    def run_iteration(self):
        ...
        if self.iteration == 0:
            print("  Generating baseline training data (relationship-first)...")
            for i, concept in enumerate(sorted(self.active_concepts)):
                try:
                    self.add_training_data_relationship_first(concept, self.baseline_defs, self.baseline_rels)
                    defs, rels = self.concept_training_level[concept]
                    print(f"    [{i+1}/{len(self.active_concepts)}] ✓ {concept}: {defs}×{rels}", flush=True)
        ...
        if self.active_concepts:
            print()
            print(f"  Adding {self.increment_defs}×{self.increment_rels} more data to {len(self.active_concepts)} concepts...")
            for concept in self.active_concepts:
                try:
                    # Add increment to accumulated data
                    self.add_training_data_relationship_first(concept, self.increment_defs, self.increment_rels)

                    # Update training level counter
                    defs, rels = self.concept_training_level[concept]
                    self.concept_training_level[concept] = (
                        defs + self.increment_defs,
                        rels + self.increment_rels
                    )
                except Exception as e:
    ...
    def run(self, max_iterations: int = 100):
        ...
        print(f"Increment: {self.increment_defs}×{self.increment_rels}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Adaptive Relationship-First Training to 95%"
    )
    ...
    parser.add_argument('--baseline-defs', type=int, default=10,
                       help='Baseline definitions per concept')
    parser.add_argument('--baseline-rels', type=int, default=10,
                       help='Baseline relationships per concept')
    parser.add_argument('--increment-defs', type=int, default=5,
                       help='Increment definitions per iteration')
    parser.add_argument('--increment-rels', type=int, default=5,
                       help='Increment relationships per iteration')
    ...
    # Initialize scaler
    scaler = AdaptiveRelationshipFirstScaler(
        model=model,
        tokenizer=tokenizer,
        concept_data=concept_data,
        output_dir=output_dir,
        target_accuracy=args.target_accuracy,
        baseline_defs=args.baseline_defs,
        baseline_rels=args.baseline_rels,
        increment_defs=args.increment_defs,
        increment_rels=args.increment_rels,
        layer_idx=-1,
        device=args.device,
