#!/usr/bin/env python3
"""
Retrain the 2 failed S-tier simplexes (taste_development, motivational_regulation).

These failed during the initial run due to CUDA OOM errors. This script:
1. Uses the same architecture and configuration as the original training
2. Reuses the existing run directory to add the missing simplexes
3. Sets PYTORCH_CUDA_ALLOC_CONF for better memory management
"""
...
# Set CUDA memory management before importing torch-based modules
...
# Lazy generation parameters (same as original)
...
def load_failed_simplexes():
    """Load only the failed simplexes."""
    with open(S_TIER_DEFS_PATH) as f:
        s_tier_defs = json.load(f)

    failed_dimensions = ['taste_development', 'motivational_regulation']
...
def main():
    print("=" * 80)
    print("RETRAIN FAILED S-TIER SIMPLEXES")
    print("=" * 80)
    print("CUDA memory management: expandable_segments=True")
    print(f"Target simplexes: taste_development, motivational_regulation")
    print(f"Existing run directory: {EXISTING_RUN_DIR}")

    # Load failed simplexes
    print("\n1. Loading failed simplexes...")
    simplexes = load_failed_simplexes()
    print(f"   Found {len(simplexes)} failed simplexes")
    for s in simplexes:
        print(f"     - {s['simplex_dimension']}")

    # Setup logging
    log_file = EXISTING_RUN_DIR / "retrain.log"
    ...
    # Train each failed simplex
    print(f"\n4. Retraining {len(simplexes)} failed simplexes...")
    ...
    for i, simplex in enumerate(simplexes, 1):
        ...
        simplex_dir = EXISTING_RUN_DIR / dimension
    ...
    # Add newly trained simplexes to graduated or failed lists
    for dim in graduated:
        if dim not in main_results['graduated']:
            main_results['graduated'].append(dim)
        if dim in main_results['failed']:
            main_results['failed'].remove(dim)

    for dim in failed:
        if dim not in main_results['failed']:
            main_results['failed'].append(dim)

    # Add new simplex results
    for result in all_results:
        ...
        for i, existing in enumerate(main_results['simplexes']):
            if existing['dimension'] == dim:
                main_results['simplexes'][i] = result
                found = True
                break
        if not found:
            main_results['simplexes'].append(result)
    ...
    print(f"\nRetrained: {len(simplexes)}")
    print(f"Graduated: {len(graduated)}/{len(simplexes)}")
    print(f"Failed: {len(failed)}/{len(simplexes)}")

    if graduated:
        print("\n✓ Newly graduated simplexes:")
        for dim in graduated:
            result = next(r for r in all_results if r['dimension'] == dim)
            print(f"  - {dim:<30} F1={result['best_test_f1']:.3f}")

    if failed:
        print("\n✗ Still failed simplexes:")
        for dim in failed:
            print(f"  - {dim}")
    ...
    total_simplexes = 13
    print(f"\n Overall: {total_graduated}/{total_simplexes} simplexes graduated")

    print(f"\n✓ Results saved to: {EXISTING_RUN_DIR}")
    print("=" * 80)
