#!/usr/bin/env python3
"""
Benchmark simplex probe accuracy for three-pole homeostatic detection.

Tests each simplex's three probes (μ−, μ0, μ+) across:
...
def load_simplex_probes(simplex_dir: Path) -> Dict[str, BinaryClassifier]:
    """
    Load all three pole probes for a simplex.
    ...
    for pole_type in ['negative', 'neutral', 'positive']:
        ...
        probe_file = pole_dir / "activation_probe.pkl"

        if not probe_file.exists():
            raise FileNotFoundError(f"Probe not found: {probe_file}")

        probe = BinaryClassifier()
        probe.load(str(probe_file))
        probes[pole_type] = probe
...
def extract_simplex_activations(
    ...
    Args:
        probes: Dict of pole_type -> BinaryClassifier
    ...
    """
    activations = {pole: [] for pole in probes.keys()}
    ...
    for prompt in prompts:
        ...
        # Get activation from all three probes
        for pole_type, probe in probes.items():
...
def extract_steered_simplex_activations(
    ...
    # Get steering vector from target pole probe
    steering_vector = probes[target_pole].coef_[0]
...
def run_simplex_benchmark(
    ...
    for prompted_pole in ['negative', 'neutral', 'positive']:
        ...
        prompt_acts = extract_simplex_activations(
            probes, prompts, model, tokenizer, device, layer_idx, context="prompt"
        )
        ...
        response_acts = extract_simplex_activations(
            probes, prompts, model, tokenizer, device, layer_idx, context="response"
        )
    ...
    for target_pole in ['negative', 'neutral', 'positive']:
        steered_acts = extract_steered_simplex_activations(
            probes, all_prompts, target_pole, model, tokenizer, device, layer_idx
        )
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark simplex probe accuracy")
    parser.add_argument("--simplexes", nargs="+", required=True, help="Simplex dimensions to benchmark")
    parser.add_argument("--probe-dir", type=Path, required=True, help="Root probe directory (e.g., results/s_tier_simplexes/run_X)")
    ...
    print("=" * 80)
    print("SIMPLEX PROBE BENCHMARK")
    print("=" * 80)

    # Default output path
    if args.output is None:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        args.output = PROJECT_ROOT / "results" / "probe_benchmarks" / f"simplex_probes_{timestamp}.csv"
    ...
    for simplex_name in tqdm(args.simplexes, desc="Benchmarking simplexes"):
        ...
        # Load probes
        simplex_dir = args.probe_dir / simplex_name
        if not simplex_dir.exists():
            print(f"  ✗ Probe directory not found: {simplex_dir}")
            continue

        try:
            probes = load_simplex_probes(simplex_dir)
            print(f"  ✓ Loaded 3 pole probes")
