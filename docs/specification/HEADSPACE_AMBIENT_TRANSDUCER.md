...
- reads the substrate’s "headspace" (internal activations) and
  transduces them into stable, concept-level probe outputs; and
...
four dimensions for each `(substrate, probe_pack)` pair it supports.
...
HATComplianceReport = {
  ...
  "locality": {
    ...
    "notes": "Probe addresses stable across minor finetunes; moderate sensitivity to layer remap."
  },
  ...
  "efficiency": {
    ...
    "max_probes_at_target_latency": 8000,
...
A Layer 2 instrumentation implementation MUST NOT be advertised as HAT-compliant for a given substrate unless it has a published, accessible HATComplianceReport for that substrate and probe pack.
...
HATComplianceReports provide the actuarial basis for treaty risk assessment. Treaty partners use these metrics to evaluate counterparty reliability, not as pass/fail certification. Probe and steering failures within documented error bounds are expected operational variance, not treaty violations.