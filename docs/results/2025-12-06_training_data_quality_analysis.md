...
This study investigates whether probe training accuracy targets (F1=0.95) are achievable given the intrinsic quality of training data generated by the subject model. We developed a quadrant-based sampling methodology to analyze concepts across varying complexity levels, used LLM judges to evaluate response relevance, and correlated these metrics with actual probe training outcomes.
...
Our probe training pipeline targets F1=0.95 with falloff validation tiers. However, overnight training runs were producing only ~500 probes with many concepts failing to graduate. This raised a fundamental question:

> Are we expecting probe accuracy that exceeds the signal-to-noise ratio in our training data?

### 1.2 Hypothesis

When a small model (8B parameters) generates text "about" a concept, the output may not strongly relate to that concept—especially for abstract or specialized terms. Probe accuracy cannot exceed the quality of the training signal.
...
We trained simple 3-layer probes on the generated samples to measure actual achievable F1 with this data quality.
...
**Key Finding**: Quadrant C has the smallest relevance gap (0.70), making it hardest for probes to distinguish positive from negative samples.

### 3.4 Probe Training Results
...
Relevance gap correlates with probe F1:
...
- A single probe attempting to capture all meanings receives a muddy, averaged signal
- Training data mixes contexts, degrading the learned decision boundary

**Proposed mitigation**: Sense-splitting—train separate probes per major synset/definition rather than one probe per term. Each probe would be trained with definition-specific prompts, capturing a coherent conceptual region.
...
- Probe must learn increasingly fine-grained distinctions
...
When both factors combine, the probe faces:
...
This explains why Quadrant C shows the lowest relevance gap (0.70) and poorest probe performance—the signal collapses under compound ambiguity.

**Note**: These mitigations (sense-splitting and sibling bucketing) would operate at the concept pack generation and training stages. They do not require hierarchical inference at runtime—probes remain independent classifiers.
