# Probe Performance Optimization Results

## Executive Summary

Through empirical testing of multiple optimization strategies, we achieved significant performance improvements in probe inference overhead:
...
|--------|---------------------|-------------------|-------------|
| Base probes loaded | 1,360 | 267 | 80.4% reduction |
...
| Initial detection | 22.68ms | 29.5% | 233 base probes @ 0.104ms/probe |
| Child detection | 1.86ms | 2.4% | 25.8 children/token @ 0.070ms/probe |
| Cache management | 0.33ms | 0.4% | Pruning logic |
| Python overhead | 0.08ms | 0.1% | Sorting, dict ops negligible |

### Key Finding

**Disk I/O became the bottleneck** - loading child probes from disk accounted for 67.6% of overhead after reducing base layers. This motivated the temporal locality caching approach.
...
1. **Active probes**: Base layers + top-k children (run every token)
2. **Warm cache**: Previously-loaded but not top-k (in memory, zero I/O cost)
3. **Cold storage**: On disk (2.4ms torch.load() cost per probe)
...
- **~70% cache hit rate** for child probes
...
**Finding**: Reducing base layers shifts overhead from initial detection to child loading with minimal overall improvement (3.7%). The bottleneck is **sequential probe inference**, not the number of base probes.
...
Pre-load child probes during prompt processing (before generation starts) to reduce generation-time disk I/O. Leverages ~51% concept overlap between prompt and generation.
...
| Probe detection | 197.63ms | Initial detection + loading |
...
Compiled each probe individually using `torch.compile(probe, mode='reduce-overhead')`.

### Performance Results

| Metric | Uncompiled (baseline) | Compiled | Result |
|--------|----------------------|----------|--------|
| Average time (267 probes) | 16.62ms | 46.79ms | **0.36x speedup** |
| Per-probe time | 0.0622ms | 0.1752ms | **3x SLOWER** |
| Compilation time | - | 1.30s | One-time cost |
| Speedup | 1.0x | **0.36x** | **181.6% WORSE** |

### Why It Failed

1. **Probes are 3-layer neural networks**, not simple linear layers
...
3. **Batched compilation also failed** due to incompatible probe architecture
...
1. **Initial detection (19.3ms, 51.0%)**
   ...
   - **Cannot batch different probes** (separately trained classifiers)
...
3. ✅ **Reduced base layers** to [0, 1] - Eliminated 80% of base probes
...
3. **Probe architecture redesign** for faster inference (if torch.compile() remains unusable)
...
- Compilation test: `scripts/benchmark_probe_compilation.py`
