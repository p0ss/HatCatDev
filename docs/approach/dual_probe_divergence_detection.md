# Dual Probe Divergence Detection

## Overview

The dual probe system detects **divergences** between what a language model internally represents (its "thoughts") and what it actually generates (its "words"). This is accomplished by training two independent types of classifiers for each semantic concept:

1. **Activation Probes** - Classify concepts from the model's internal hidden states
2. **Text Probes** - Classify concepts from the generated token text

When these two probes disagree significantly, we detect a **divergence** - the model is thinking one thing but writing another.
...
Both probe types are trained **independently** with separate learning curves and graduation criteria:

```python
# Activation probe: slower, more cautious
samples_per_iteration_activation = 1

# Text probe: faster, more aggressive
samples_per_iteration_text = 5

# Both graduate at 95% F1, max 50 iterations
```

This allows each probe type to find its own optimal sample size for the data it's working with.

## Divergence Measurement

For each generated token, we:

1. Run the **activation probe** on the hidden state → get P(concept|hidden_state)
2. Run the **text probe** on the token text → get P(concept|token_text)
...
Trained **5,582 dual probe pairs** across 6 semantic layers:
...
### Non-Graduating Probes

Some text probes don't reach 95% F1 within 50 iterations but are still saved and usable at 75-92% accuracy. These represent concepts where surface text is a weak signal (e.g., abstract concepts like "Process" or "Relation").
...
The dual probe system successfully detects this phenomenon - the model "thinks" in rich semantic concepts but "writes" in sparse surface tokens.
...
### Monitoring
- `src/monitoring/dynamic_probe_manager.py` - Dynamic loading/unloading of dual probes
...
### Results
- `results/sumo_classifiers_adaptive_l0_5/` - Trained dual probes for layers 0-5
...
Each concept has two probe files:

```
results/sumo_classifiers_adaptive_l0_5/
  layer0/
    Physical/
      activation_probe.pth      # SimpleMLP for hidden states
      text_probe.pkl            # TF-IDF + LogReg for tokens
      metadata.json             # Training stats, F1 scores, sample counts
```

### Dynamic Loading

The `DynamicProbeManager` loads probes on-demand to handle 5,582 concepts efficiently:

- **Base layers** (layer 0): Always loaded (14 probes)
- **Child concepts**: Loaded when parent activates
- **Top-K pruning**: Keep only highest-scoring probes in memory (max 500)
- **Separate pools**: Activation probes (GPU) and text probes (CPU)
...
1. **Per-token training**: Train probes on individual token positions vs mean-pooled activations
...
If you use this dual probe divergence detection system, please cite:

```
HatCat: Dual Probe Divergence Detection for Language Model Interpretability
