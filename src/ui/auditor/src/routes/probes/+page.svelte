<script lang="ts">
	...
	import type { ExperienceTick, ProbeTrace } from '$lib/types';
	...
	// Build probe traces from recent ticks
...
<div class="probes-page">
	...
	<div class="panels">
		...
		<div class="panel simplex-panel">
			<h3>Simplex Probes (Proprioception)</h3>
					...
					{#each simplexTraces as [term, data]}
						...
						<div class="probe-row">
							<div class="probe-info">
								<span class="probe-name">{term}</span>
								<span class="probe-stats">
									{s.avg.toFixed(3)} (±{(s.max - s.min).toFixed(3)})
								</span>
								{#if lastDev !== null && lastDev !== undefined}
									<span
										class="probe-deviation"
			...
			{:else}
				<div class="probe-list">
					{#each conceptTraces as [concept, data]}
						...
						<div class="probe-row">
							<div class="probe-info">
								<span class="probe-name">{concept}</span>
								<span class="probe-stats">
...
<style>
	...
	.probe-deviation.warning {
		color: #f0883e;
	}

	.probe-deviation.danger {
		color: #f85149;
	}
