"""
Probe validation to detect poorly-calibrated classifiers.

Validates that probes fire specifically on their target domain,
...
def validate_probe_calibration(
    ...
    """
    Validate that a probe fires specifically on its target domain.

    Tests the probe on diverse prompts and checks:
    1. Does it fire strongly on expected domain?
    2. Does it fire weakly on other domains?

    A well-calibrated probe should rank high on target domain and
    ...
    Args:
        ...
        top_k: Number of top probes to check
    ...
    Returns:
        ...
        - passed: Whether probe passed validation
    """
    from .classifier import BinaryClassifier

    # Load probe
    hidden_dim = 2560  # Gemma-3-4b
    probe = BinaryClassifier(hidden_dim).to(device)
...
def validate_probe_set(
    ...
    """
    Validate all probes in a directory.

    Args:
        probe_dir: Directory containing probe .pt files
    ...
    Returns:
        Dict with summary statistics and per-probe results
    """
    ...
    print(f"Validating {len(probe_files)} probes...")

    for i, probe_path in enumerate(probe_files):
        concept_name = probe_path.stem.replace('_classifier', '')

        if concept_name not in concept_map:
            print(f"  [{i+1}/{len(probe_files)}] Skipping {concept_name} (no metadata)")
            continue

        concept = concept_map[concept_name]

        try:
            result = validate_probe_calibration(
                probe_path, concept, model, tokenizer, device, layer_idx
            )
            results.append(result)

            status = "✓ PASS" if result['passed'] else "✗ FAIL"
            print(f"  [{i+1}/{len(probe_files)}] {concept_name}: {status} "
            ...
            print(f"  [{i+1}/{len(probe_files)}] ERROR validating {concept_name}: {e}")
            failed += 1
    ...
    # Save results
    if save_results:
        output_file = probe_dir / 'validation_results.json'
        import json
    ...
    failed_probes = [r for r in results if not r['passed']]
    if failed_probes:
        print(f"\nWorst calibrated probes (universal firing):")
        failed_probes.sort(key=lambda r: r['avg_other_rank'])
        for r in failed_probes[:10]:
