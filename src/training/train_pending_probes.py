#!/usr/bin/env python3
"""
Train pending probes from pending_training.json.

This script trains probes for concepts that were added via meld but haven't
been trained yet. It reads pending_training.json, trains the required probes,
and updates the version_manifest.json.

Usage:
    python scripts/train_pending_probes.py \
        --concept-pack sumo-wordnet-v4 \
        --probe-pack probe_packs/apertus-8b_sumo-wordnet-v4 \
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train pending probes from pending_training.json"
    )
...
def main():
    ...
    probe_pack_dir = Path(args.probe_pack)
    ...
    for layer, concepts in sorted(by_layer.items()):
        ...
        layer_output_dir = probe_pack_dir / f"layer{layer}"
    ...
    print("=" * 80)
    print()
    print(f"Trained: {len(all_trained)} probes")
    print(f"Manifest updated: {probe_pack_dir / 'version_manifest.json'}")
